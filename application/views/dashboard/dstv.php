...
<div class="col mt-3">
                                	<!--Form-->
                 					<h2 class="mb-2">Pay DStv Subscription<span style="font-weight: 400"> 
									- Instant Activation</span></h2>           
									<?php $attributes = array('id' => 'form'); ?>
                                <?= form_open('',$attributes); ?>       
		                         
		                            <div class="form-group">
		                                <label for="city">DSTV Subscription</label>
		                                <select name="cboCableTV" id="cboCableTV" onchange="GetAmount();" onmouseup="GetAmount()" onmousedown="GetAmount()" class="form-control" required>		<option selected disabled>-- Select One -- </option>
		                                		<option value="dstv">DStv                                  
		                                </select>
		                            </div>
		                            <div class="form-group">
		                                <label for="city">Bouquet/Package</label>
		                                <select name="cboPackage" id="cboPackage" tabindex="3" onchange="GetAmount2();" onmouseup="GetAmount2()" onmousedown="GetAmount2()" class="form-control" required>
		                            ...
		                            <div class="form-group">
		                                ...
		                                <div  id="lblSmartCardNo" name="lblSmartCardNo"></div>		                                
		                                <input type="number" name="iuc in-md" id="iuc" class="form-control input-md in-md" placeholder="Enter Iuc number" required>
		                                
		                                  
		                            </div>		                            
		                           <div class="form-group">
		                          <label>Price</label>
		                          <input type="text" name="txtAmount" id="txtAmount" value="" class="form-control in-md" readonly="true">
		                      </div>
		                      
		                            <div class="form-group">
		                              <input type="hidden" id="input_iuc">
		                            
		                               	<button type="button" onclick="topup()" class="btn btn-primary btn-block" name="cmdSubmit" id="cmdSubmit" value="Click to Proceed" >Pay Now</button>
		                            </div>		                            
									<!--/Form-->
									<?= form_close();?>
		                            <div class="row">
								...
								<script src="<?= base_url();?>asset/js/verfiyiuc.js"></script>
...
<script type="text/javascript">
								...
								i='2';j='7';
								arrCode[i][j]='smart-weekly';
								arrAmount[i][j]='700.00';
								if(i==2)
								{
									arrName[i][j]='Smart ';
								}
...
<script>
			...
			dataType : 'json',
                    success: function(data){
                        ...
                        if(data == "INVALID_SMARTCARDNO"){
                            ...
                            $('#input_iuc').val(data);
							var error;
							error +="<p style='color:blue'>"+ data +"</p>";
                            $('#lblSmartCardNo').html(error);
                        }
},
                 error:function(data){
    ...
    $('#cmdSubmit').submit((e)=>{
	  e.preventDefault();
	  var iuc = $('#input_iuc').val(data);
	  var iuc_form = $("#iuc").val();
		$(this).val("Please Wait......");
		if(iuc !="" ){
			$(this).attr('type','submit');
		}else{
			iuc_form.text("Kindly Provide Correct Smart card Number");
		}
